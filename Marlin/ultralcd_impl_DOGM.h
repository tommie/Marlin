...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
    ...
    u8g.drawBitmapP(
      STATUS_SCREEN_X, 1,
      STATUS_SCREENBYTEWIDTH, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
  ...
  if (PAGE_UNDER(28)) {
    // Extruders
    HOTEND_LOOP() _draw_heater_status(STATUS_SCREEN_HOTEND_TEXT_X(e), e);

    // Heated bed
    #if HOTENDS < 4 && HAS_TEMP_BED
      _draw_heater_status(STATUS_SCREEN_BED_TEXT_X, -1);
    #endif
